...
# Valores podem ser distância ou tempo entre as cidades
grafo = [[0, 2, 6, 3], [1, 0, 2, 5], [4, 2, 0, 1], [6, 9, 1, 0]]
# [1, 4, 3, 2, 1]
# 3 + 1 + 2 + 1 = 7

# grafo = [[0, 10, 15, 20], [10, 0, 35, 25], [15, 35, 0, 30], [20, 25, 30, 0]]
# [1, 3, 4, 2, 1]
# 15 + 30 + 25 + 10 = 80
...
def caixeiro_viajante_MTZ(grafo, circuito=False):
    qntd_pontos = len(grafo)
    origens = [i + 1 for i in range(qntd_pontos)]
    destinos = [i + 1 for i in range(qntd_pontos)]

    custos = dict()
    for i, origem in enumerate(origens):
        for j, destino in enumerate(destinos):
            custos[origem, destino] = grafo[i][j]

    m = gp.Model()

    # Variáveis decisão
    x = m.addVars(origens, destinos, vtype=gp.GRB.BINARY, name="x")
    u = m.addVars(origens[1:], vtype=gp.GRB.INTEGER, ub=qntd_pontos - 1, name="u")

    # Função Objetivo (Multiplicação dos custos_ij por x_ij)
    m.setObjective(x.prod(custos), sense=gp.GRB.MINIMIZE)

    # Restrições
    m.addConstrs(gp.quicksum(x[i, j] for j in destinos if i != j) == 1 for i in origens)
    m.addConstrs(gp.quicksum(x[i, j] for i in origens if i != j) == 1 for j in destinos)
    m.addConstrs(
        u[i] - u[j] + qntd_pontos * x[i, j] <= qntd_pontos - 1
        for i in origens[1:]
        for j in destinos[1:]
        if i != j
    )
    ...
    print(f"Optimal obj value: {m.objVal}")

    print("valores u :", u.values)

    if circuito:
        circuito_result = [1]
        anterior = 1
        for _ in range(qntd_pontos):
            for j in destinos:
                if round(x[anterior, j].X) == 1:
                    circuito_result.append(j)
                    anterior = j
                    break
        print("\n")
        print(circuito_result)


caixeiro_viajante_MTZ(grafo, True)
